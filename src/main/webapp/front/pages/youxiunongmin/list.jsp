  ...
  <div id="app">
    ...
    <fieldset class="search-container" style="text-align: center;margin: 50px 0;">
      <form class="layui-form" style="margin: 10px 0;">
        <div class="layui-inline">
          <label class="layui-form-label">农民姓名</label>
          <div class="layui-input-inline">
            <input type="text" name="nongminxingming" id="nongminxingming" placeholder="农民姓名" autocomplete="off" class="layui-input">
          </div>
        </div>   
        <div class="layui-inline" style="margin-left: 30px;">
          ...
          <button v-if="isAuth('youxiunongmin','新增')" @click="jump('../youxiunongmin/add.jsp')" type="button" class="layui-btn btn-theme">
    ...
    <div class="product">
      <div class="proBox" v-for="(item,index) in dataList" v-bind:key="index">
        ...
        <div class="proBoxtxt">
          <p>{{item.nongminxingming}}&nbsp;&nbsp;<span style="color: red;" v-if="item.price">{{item.price}} RMB</span></p>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#nongminxingming').val()) {
          param['nongminxingming'] = jquery('#nongminxingming').val() ? '%' + jquery('#nongminxingming').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('youxiunongmin/list', 'get', param, function(res) {
